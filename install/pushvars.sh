...
tfh pushvars -org PublicSector-ATARC -name fse-tf-atarc-boundary-config -svar "psql_user=${PSQL_USER}" -overwrite psql_user -token $TFC_TOKEN
tfh pushvars -org PublicSector-ATARC -name fse-tf-atarc-boundary-config -svar "psql_pw=${PSQL_PW}" -overwrite psql_pw -token $TFC_TOKEN
